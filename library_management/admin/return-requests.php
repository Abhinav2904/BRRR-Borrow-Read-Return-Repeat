...
<body>
                ...
                <table class="table table-borderless">
                    ...
                    <tbody id="borrowed-books-list">
                        ...
                        <tr class="borrowed-book-item">
                            ...
                            <td>
                            <button class="btn btn-sm btn-success accept-btn" data-id="<?php echo $row['id']; ?>" data-book-id="<?php echo $row['book_id']; ?>" onclick="handleReturnRequest(this)">Accept</button>
    ...
    <script>
        function handleReturnRequest(button) {
    const borrowId = button.getAttribute('data-id');
    const bookId = button.getAttribute('data-book-id');

    fetch('update-return-status.php', {
        method: 'POST',
        headers: {
        ...
        .then(data => {
            if (data.success) {
                // Replace the button with "Returned" text
                button.outerHTML = `<span class="text-success">Returned</span>`;
            } else {
                alert('Failed to update the status. Error: ' + (data.error || 'Unknown error'));
            }
        })
        .catch(error => {
