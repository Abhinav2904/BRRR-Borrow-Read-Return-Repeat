...
<body>
                ...
                <table class="table table-borderless">
                    ...
                    <tbody id="borrowed-books-list">
                        ...
                        <tr class="borrowed-book-item">
                            ...
                            <td>
                                <?php if($row['status']=='1'){?>
                                    <button class="btn btn-sm btn-primary return-btn" data-id="<?php echo $row['id']; ?>">Return</button>
                                <?php }else{?>
                                    <span class="text-success">Return Requested</span>
                                <?php }?>
                            </td>
                        </tr>
                        <?php } endwhile; ?>
    ...
    button.addEventListener('click', function () {
        const entryId = this.getAttribute('data-id'); // Get the borrowed_books entry ID

        // Perform AJAX request
        fetch('update-borrow-status.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ id: entryId, status: 3 }) // Update status to 4
        })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    // Replace the button with green text
                    const parent = this.parentElement;
                    parent.innerHTML = '<span class="text-success">Return Requested</span>';
                } else {
